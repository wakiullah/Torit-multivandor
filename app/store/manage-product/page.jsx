...
export default function StoreManageProducts() {
    ...
    const storeId = user?.store?._id || user?.store; // Handle both populated object or direct ID
                ...
                <tbody className="text-slate-700">
                    {products.map((product) => (
                        <tr key={product.id} className="border-t border-gray-200 hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3 hidden md:table-cell">{currency} {product.mrp ? product.mrp.toLocaleString() : 'N/A'}</td>
                            <td className="px-4 py-3">{currency} {product.price ? product.price.toLocaleString() : 'See variations'}</td>
                                                        <td className="px-4 py-3 text-center flex items-center gap-2">
                                                            <button onClick={() => openEditModal(product)} className="text-blue-600 hover:text-blue-800 text-sm">Edit</button>
                                                            <button onClick={() => deleteProduct(product.id)} className="text-red-600 hover:text-red-800 text-sm">Delete</button>
